...
* 시간복잡도 : 알고리즘 수행 시간을 나타내는 지표로 주로 Big-O를 사용. 
* 공간복잡도 : 알고리즘 메모리 사용량을 나타내는 지표

### 꼬리질문
1. Big-O, Big-Theta, Big-Omega 에 대해 설명해 주세요.
- Big-O: 최악의 경우를 표현, 해당 알고리즘이 big-O로 표현된 시간보다 빠르기만 하면 된다. 최대로 걸릴 수 있는 시간(상한 시간)
- big-θ:  O와 Ω를 둘 다 만족하는 경우를 의미
- Big-Ω:   최소로 걸릴 수 있는 시간(하한 시간). 최선의 경우 시간복잡도.
### 다른 것을 사용하지 않고, Big-O를 사용하는 이유가 있을까요?
- 최선의 상황을 마주하기 보단 최악의 상황을 마주하는 경우가 많기 때문.


## 2. 링크드 리스트에 대해 설명해 주세요.
- 노드간 연결되어 있는 구조. 각 노드는 다음 노드를 가리키는 링크가 존재. 헤드와 테일의 위치 근처에선 삽입과 삭제가 빠르게 수행됨. 메모리 효율적.
